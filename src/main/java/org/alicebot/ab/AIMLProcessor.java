...
public class AIMLProcessor {
  ...
  private static void categoryProcessor(Node n, ArrayList<Category> categories, String topic, String aimlFile, String language) {
    ...
    for (int j = 0; j < children.getLength(); j++) {
      // logger.debug("CHILD: " + children.item(j).getNodeName());
      ...
      if (mName.equals("#text")) {
        ...
        logger.debug("categoryProcessor: unexpected " + mName + " in " + DomUtils.nodeToString(m));
    }
    // logger.debug("categoryProcessor: pattern="+pattern);
    ...
    /*
     * if (template == null) logger.debug("Template is null"); if (template.length()==0) logger.debug("Template is zero length");
     */
    if (template == null || template.length() == 0) {
      logger.debug("Category " + c.inputThatTopic() + " discarded due to blank or missing <template>.");
      ;
    } else
      categories.add(c);
  }
  ...
  public static ArrayList<Category> AIMLToCategories(String directory, String aimlFile) {
    try {
      ...
      logger.debug("AIMLToCategories: " + ex);
      ex.printStackTrace();
      return null;
    }
  }
  ...
  public static int checkForRepeat(String input, Chat chatSession) {
    if (input.equals(chatSession.getInputHistory().get(1))) {
      return 1;
    } else
      return 0;
  }
  ...
  public static String respond(String input, String that, String topic, Chat chatSession, int srCnt) {
    MagicBooleans
        .trace("input: " + input + ", that: " + that + ", topic: " + topic + ", chatSession: " + chatSession + ", srCnt: " + srCnt);
    String response;
  ...
  /**
   * return a string of unevaluated XML. When the AIML parser encounters an unrecognized XML tag, it simply passes through the tag in XML
   * form. For example, if the response contains HTML markup, the HTML is passed to the requesting process. However if that markup contains
   * AIML tags, those tags are evaluated and the parser builds the result.
   *
   * @param node
   *          current parse node.
   * @param ps
  ...
  /**
   * in AIML 2.0, an attribute value can be specified by either an XML attribute value or a subtag of the same name. This function tries to
   * read the value from the XML attribute first, then tries to look for the subtag.
   *
   * @param node
   *          current parse node.
   * @param ps
  ...
  private static String getAttributeOrTagValue(Node node, ParseState ps, String attributeName) { // AIML 2.0
    ...
    if (m == null) {
      ...
      for (int i = 0; i < childList.getLength(); i++) {
        Node child = childList.item(i);
        // logger.debug("getAttributeOrTagValue child = "+child.getNodeName());
        if (child.getNodeName().equals(attributeName)) {
          result = evalTagContent(child, ps, null);
          // logger.debug("getAttributeOrTagValue result from child = "+result);
        }
      }
    } else {
  ...
  private static String set(Node node, ParseState ps) { // add pronoun check
    ...
    if (predicateName != null) {
      ps.chatSession.getPredicates().put(predicateName, result);
  ...
  private static String get(Node node, ParseState ps) {
    ...
    if (predicateName != null)
      result = ps.chatSession.getPredicates().get(predicateName).trim();
  ...
  public static String tupleGet(String tupleName, String varName) {
    ...
    // logger.debug("Tuple = "+tuple.printTuple());
    // logger.debug("Value = "+tuple.getValue(varName));
    if (tuple == null)
  ...
  /**
   * <interval><style>years</style></style><jformat>MMMMMMMMM dd, yyyy</jformat><from>August 2, 1960</from><to><date><jformat>MMMMMMMMM dd,
  ...
  private static String inputStar(Node node, ParseState ps) {
    ...
    // logger.debug("inputStar: ps depth="+ps.depth+" index="+index+" star="+result);
    return result;
  }
  ...
  /**
   * implements <topicstar/> and <topicstar index="N"/> returns the value of input words matching the Nth wildcard (or AIML Set) in a topic
   * pattern.
   *
   * @param node
   *          current XML parse node
  ...
  private static String id(Node node, ParseState ps) {
    return ps.chatSession.getCustomerId();
  }
  ...
  private static String that(Node node, ParseState ps) {
    ...
    if (value != null)
      try {
        ...
        logger.debug("That index=" + index + "," + jndex);
      } catch (Exception ex) {
        ex.printStackTrace();
      }
    String that = MagicStrings.unknown_history_item;
    History hist = ps.chatSession.getThatHistory().get(index);
    if (hist != null)
  ...
  private static String input(Node node, ParseState ps) {
    int index = getIndexValue(node, ps);
    return ps.chatSession.getInputHistory().getString(index);
  }
  ...
  private static String request(Node node, ParseState ps) { // AIML 2.0
    int index = getIndexValue(node, ps);
    return ps.chatSession.getRequestHistory().getString(index).trim();
  }
  ...
  private static String response(Node node, ParseState ps) { // AIML 2.0
    int index = getIndexValue(node, ps);
    return ps.chatSession.getResponseHistory().getString(index).trim();
  }

  /**
   * implements {@code <system>} tag. Evaluate the contents, and try to execute the result as a command in the underlying OS shell. Read
   * back and return the result of this command.
   *
  ...
  /**
   * Transform a string of words (separtaed by spaces) into a string of individual characters (separated by spaces). Explode "ABC DEF" =
   * "A B C D E F".
   *
   * @param node
   *          current XML parse node
  ...
  private static String condition(Node node, ParseState ps) {
    ...
    if (liList.size() == 0 && (value = getAttributeOrTagValue(node, ps, "value")) != null &&
        predicate != null &&
        ps.chatSession.getPredicates().get(predicate).equalsIgnoreCase(value)) {
    ...
    // otherwise this is a <condition> with <li> items:
      ...
      for (int i = 0; i < liList.size() && result.equals(""); i++) {
        ...
        // logger.debug("condition name="+liPredicate+" value="+value);
        if (value != null) {
          // if the predicate equals the value, return the <li> item.
          if (liPredicate != null && value != null && (ps.chatSession.getPredicates().get(liPredicate).equalsIgnoreCase(value) ||
              (ps.chatSession.getPredicates().containsKey(liPredicate) && value.equals("*"))))
  ...
  private static String deleteTriple(Node node, ParseState ps) {
    ...
    return ps.chatSession.getTripleStore().deleteTriple(subject, predicate, object);
  }
  ...
  private static String addTriple(Node node, ParseState ps) {
    ...
    String predicate = getAttributeOrTagValue(node, ps, "pred");
    String object = getAttributeOrTagValue(node, ps, "obj");
    return ps.chatSession.getTripleStore().addTriple(subject, predicate, object);
  }

  /*
   * private static String qnotq(Node node, ParseState ps, Boolean affirm) { String subject = getAttributeOrTagValue(node, ps, "subj");
   * String predicate = getAttributeOrTagValue(node, ps, "pred"); String object = getAttributeOrTagValue(node, ps, "obj"); TripleStore ts =
   * ps.chatSession.getTripleStore(); HashSet<String> triples = ts.getTriples(subject, predicate, object); if (!affirm) { HashSet<String> U
   * = new HashSet<String>(ts.allTriples()); U.removeAll(triples); triples = U; } return ts.formatAIMLTripleList(triples); } private static
   * String q(Node node, ParseState ps) { return qnotq(node, ps, true); } private static String notq(Node node, ParseState ps) { return
   * qnotq(node, ps, false); }
   */
  public static String uniq(Node node, ParseState ps) {
    ...
    HashSet<Tuple> tuples = ps.chatSession.getTripleStore().selectFromSingleClause(partial, clause, true);
    String tupleList = "";
  ...
  public static String select(Node node, ParseState ps) {
    ...
    for (int i = 0; i < childList.getLength(); i++) {
      ...
      if (childNode.getNodeName().equals("vars")) {
        ...
        // logger.debug("AIML Processor select: visible vars "+visibleVars);
      }
        ...
        // logger.debug("Vars "+vars+" Clause "+subj+" "+pred+" "+obj+" "+affirm);
        clauses.add(clause);

      }
    }
    HashSet<Tuple> tuples = ps.chatSession.getTripleStore().select(vars, visibleVars, clauses);
    String result = "";
    for (Tuple tuple : tuples) {
  ...
  public static String subject(Node node, ParseState ps) {
    ...
    TripleStore ts = ps.chatSession.getTripleStore();
  ...
  public static String resetlearnf(Node node, ParseState ps) {
    ps.chatSession.getBot().deleteLearnfCategories();
    return "Deleted Learnf Categories";

  }

  public static String resetlearn(Node node, ParseState ps) {
    ps.chatSession.getBot().deleteLearnCategories();
  ...
  public static boolean validTemplate(String template) {
    ...
    try {
      ...
      // e.printStackTrace();
      logger.debug("Invalid Template " + template);
      return false;
    }
