...
public class Sraix {
  ...
  public static String sraix(Chat chatSession, String input, String defaultResponse, String hint, String host, String botid, String apiKey, String limit, final String plugin) {
    ...
    userinteraction.outputForUserWithNewline("Sraix: response = " + response + " defaultResponse = " + defaultResponse);
  ...
  public static String sraixPandorabots(String input, Chat chatSession, String host, String botid) {
    // logger.debug("Entering SRAIX with input="+input+" host ="+host+" botid="+botid);
  ...
  public static String pandorabotsRequest(String input, String host, String botid) {
    try {
      ...
      // logger.debug("Pandorabots Request "+input);
      ...
      /*
       * String scheme = "http"; String authority = host; String path = "/pandora/talk-xml"; String query =
       * "botid="+botid+"&custid="+custid+"&input="+input; String fragment = null; URI uri=null; String out; try { uri = new URI(scheme,
       * authority, path, query, fragment); out = "\n"; out += "URI example:\n"; out += "        URI string: "+uri.toString()+"\n";
       * userinteraction.outputForUser(out); } catch (Exception e) { e.printStackTrace(); }
       */
      // uri = new URI(spec);
      ...
      // logger.debug("Sraix: Response="+responseContent);
...
//          + "&exclude=Dialogues,ChatBot&out=json&clientFeatures=show-images,reminder,say&debug=true";
      ...
      if (page == null || page.length() == 0) {
        ...
        if (outputJson.length() == 0) {
          ...
          // MagicBooleans.trace("in Sraix.sraixPannous, firstHandler class: " + firstHandler.getClass() + ", firstHandler: " +
          // firstHandler);
      ...
      ex.printStackTrace();
      logger.debug("Sraix '" + input + "' failed");
    }
    return MagicStrings.sraix_failed;
  } // sraixPannous

  public static void log(String pattern, String template) {
    logger.debug("Logging " + pattern);
