...
public class AB {
  ...
  private UserInteraction userinteraction;
    ...
    public AB(Bot bot, String sampleFile) {
        ...
        logger.debug("AB with sample file "+logfile);
        this.bot = bot;
    ...
    public void productivity (int runCompletedCnt, Timer timer) {
        ...
        logger.debug("Completed "+runCompletedCnt+" in "+time+" min. Productivity "+(float)runCompletedCnt/time+" cat/min");
    }
    public void readDeletedIFCategories() {
        bot.readCertainIFCategories(deletedGraph, MagicStrings.deleted_aiml_file);
        if (MagicBooleans.trace_mode) logger.debug("--- DELETED CATEGORIES -- read "+deletedGraph.getCategories().size()+" deleted categories");
    }
    public void writeDeletedIFCategories() {
        logger.debug("--- DELETED CATEGORIES -- write");
        bot.writeCertainIFCategories(deletedGraph, MagicStrings.deleted_aiml_file);
        logger.debug("--- DELETED CATEGORIES -- write "+deletedGraph.getCategories().size()+" deleted categories");
    ...
    public void saveCategory(String pattern, String template, String filename) {
        ...
        else logger.debug("Invalid Category "+c.validationMessage);
    }
    ...
    public void deleteCategory(Category c) {
        ...
        logger.debug("--- bot.writeDeletedIFCategories()");
    ...
    public void skipCategory(Category c) {
       /* c.setFilename(MagicStrings.unfinished_aiml_file);
        ...
        logger.debug(bot.unfinishedGraph.getCategories().size() + " unfinished categories");
    ...
    public void abwq() {
        ...
        logger.debug(timer.elapsedTimeSecs() + " classifying inputs");
        bot.writeQuit();
    }
    ...
    public void graphInputs (String filename) {
        ...
        try{
            ...
            while ((strLine = br.readLine()) != null  && count < limit)   {
                ...
                count++;
                //logger.debug("Root branches="+g.root.size());
            }
    ...
    void findPatterns(Nodemapper node, String partialPatternThatTopic) {
        if (NodemapperOperator.isLeaf(node)) {
            //logger.debug("LEAF: "+node.category.getActivationCnt()+". "+partialPatternThatTopic);
            if (node.category.getActivationCnt() > MagicNumbers.node_activation_cnt) {
                //logger.debug("LEAF: "+node.category.getActivationCnt()+". "+partialPatternThatTopic+" "+node.shortCut);    //Start writing to the output stream
                leafPatternCnt ++;
                try {
                    String categoryPatternThatTopic = "";
                    if (node.shortCut) {
                        //logger.debug("Partial patternThatTopic = "+partialPatternThatTopic);
        ...
        if(NodemapperOperator.size(node) > MagicNumbers.node_size) {
            //logger.debug("STAR: "+NodemapperOperator.size(node)+". "+partialPatternThatTopic+" * <that> * <topic> *");
            starPatternCnt ++;
            try {
    ...
    public void classifyInputs (String filename) {
        try{
            ...
            while ((strLine = br.readLine())!= null && count < limit)   {
                ...
                //logger.debug("Classifying "+strLine);
                ...
                if (strLine != null) {
                    ...
                    for (int i = 0; i < sentences.length; i++) {
                        ...
                        if (sentence.length() > 0) {
                            ...
                            if (match == null) {
                                userinteraction.outputForUserWithNewline(sentence+" null match");
                            }
                            else {
                                match.category.incrementActivationCnt();
                                //userinteraction.outputForUserWithNewline(count+". "+sentence+" matched "+match.category.inputThatTopic());
                            }
                            count += 1;
                            if (count%10000 == 0) userinteraction.outputForUserWithNewline(count);
                        }
                    }
                }
            }
            logger.debug("Finished classifying "+count+" inputs");
            //Close the input stream
            br.close();
        } catch (Exception e){//Catch exception if any
            System.err.println("Error: " + e.getMessage());
    ...
    public void ab () {
        ...
        graphInputs(logFile);
        logger.debug(timer.elapsedTimeSecs() + " seconds Graphing inputs");
        ...
        timer.start();
        logger.debug("Finding Patterns");
        findPatterns();
        logger.debug(suggestedCategories.size()+" suggested categories");
        logger.debug(timer.elapsedTimeSecs() + " seconds finding patterns");
        timer.start();
        patternGraph.nodeStats();
        if (patternGraph.getCategories().size() < MagicNumbers.brain_print_size) patternGraph.printgraph();
        logger.debug("Classifying Inputs from "+logFile);
        classifyInputs(logFile);
        logger.debug(timer.elapsedTimeSecs() + " classifying inputs");
    }
    ...
    public ArrayList<Category> nonZeroActivationCount(ArrayList<Category> suggestedCategories) {
        ...
        for (Category c : suggestedCategories) {
           ...
           // else     logger.debug("["+c.getActivationCnt()+"] "+c.inputThatTopic());
        }
    ...
    public void terminalInteraction () {
       ...
       // if (sort_mode)
        ...
        // logger.debug(filteredAtomicCategories.size()+" filtered suggested categories");
        ...
        for (Category c : browserCategories)  {
            ...
            Nodemapper node;
                if (offer_alice_responses) {
                    ...
                    if (node != null) {
                        ...
                        userinteraction.outputForUserWithNewline("ALICE: "+displayAliceTemplate);
                    }
            ...
            terminalInteractionStep(bot, "", textLine, c, alicetemplate);
                ...
                ex.printStackTrace();
                logger.debug("Returning to Category Browser");
            }
        }
   ...
   public void terminalInteractionStep (Bot bot, String request, String textLine, Category c, String alicetemplate) {
       ...
       if (textLine.contains("<pattern>") && textLine.contains("</pattern>")) {
           ...
           if (index < jndex) {
               ...
               logger.debug("Got pattern = "+pattern+" template = "+textLine);
           }
       }
       String botThinks = "";
